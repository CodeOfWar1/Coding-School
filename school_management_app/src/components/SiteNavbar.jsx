...
export default function SiteNavbar({
  ...
  const shell =
    variant === 'light'
      ? `${stickyCls} border-b border-slate-200/90 bg-white/95 shadow-sm backdrop-blur-md`
      : `${stickyCls} border-b border-white/10 bg-slate-950/45 backdrop-blur-xl`
      ...
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <Link to="/" className="flex min-w-0 items-center gap-3 text-left transition hover:opacity-95">
          <img
            src="/media/Logo.png"
            alt=""
            className={`h-10 w-10 shrink-0 rounded-lg p-0.5 ring-1 ${
              variant === 'light' ? 'bg-white ring-slate-200' : 'bg-white/10 ring-white/20'
            }`}
          />
          <div className="min-w-0">
            <p className={`text-xs font-medium uppercase tracking-wide ${variant === 'light' ? 'text-sky-700' : 'text-white/75'}`}>
              Mandakh
            </p>
            <p
        ...
        <nav className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
          ...
          {showRegisterPay && onRegisterPay && (
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-bold shadow-md transition ${
                variant === 'light'
